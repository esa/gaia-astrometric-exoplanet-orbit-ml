...
def __decision_plot_matplotlib(
    ...
    fontsize = 11 if s is None else 9
    ...
    if (cumsum.shape[0] == 1) and (features is not None):
        ...
        for i in range(feature_display_count):
            ...
            if isinstance(v, str):
                ...
                v = "({})".format(f"{v:,.1f}".rstrip("0").rstrip("."))
            t = ax.text(
    ...
    pl.xlabel("Model Outlier Score (0 - 1)", fontsize=13)
...
def decision(
    ...
    if isinstance(link, LogitLink):
        ...
        if a > b:
            xlim = (0, base_value + a * 1.2)
        else:
            xlim = (0, base_value + b * 1.2)
